...
export default function SignUp() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            await axios.post('http://127.0.0.1:8000/api/signup/', { firstname, lastname, username, password, email, gender, time_zone })
            navigate('/login')
        } catch (err) {
            console.error(err)
            alert('Signup failed')
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <input placeholder='Email' value={email} onChange={e => setEmail(e.target.value)} />
            {
                zones.length
                    ?
                    <select value={time_zone} onChange={e => setTimezone(e.target.value)}>
                        <option value={''}></option>
                        {
                            zones.map(timezone => {
                                return (
